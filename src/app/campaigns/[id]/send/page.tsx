...
export default function SendPage() {
  ...
  if (!campaign || !template || !settings || !report) return null;
  ...
  async function runSimulation() {
    ...
    const html = renderEmailHtml({ template: template!, buyer, settings: settings!, assetsBySlot });
    const text = renderEmailText({ template: template!, buyer, settings: settings!, assetsBySlot });
    ...
    await logActivity(
      "email.simulated",
      `Simulated test send for ${buyer.company || buyer.email}`,
      ...
      <div className="mt-6 rounded-2xl bg-brand-canvas border border-brand-border p-6">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button className="btn-primary" onClick={runSimulation} disabled={simulating || recipientBuyers.length === 0}>
        ...
        {testResult && (
          <div className="p-6">
            <div className="rounded-xl bg-emerald-50 border border-emerald-100 px-4 py-3 text-[13px] text-emerald-800 mb-4">
              <span className="font-medium">Email successfully prepared.</span> Live Gmail sending will be connected in Phase 2.
            </div>
