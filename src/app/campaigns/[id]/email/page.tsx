...
import type { Buyer, EmailSection } from "@/lib/types";
...
export default function EmailComposerPage() {
  ...
  async function save() {
    if (!draftSections || !template) return;
    setSaving(true);
    await templateRepo.update(template.id, { sections: draftSections });
    await logActivity("template.updated", `Template "${template.name}" saved`);
            ...
            return (
              <li key={s.id}>
                ...
                >
                  ...
                  <span className="flex-1 truncate">{SECTION_LABELS[s.type]}</span>
                  <button
